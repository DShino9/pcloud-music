...
「🎰 ジューク」から。棚の絞り込みは持ち込まず、**筐体の中でジャンルと雰囲気を選ぶ**。

- **ジャンル** … 棚に付いている札から起こす
- **雰囲気** … `moodOf()` の結果から起こすので、**手で付けた札もそのまま選べる**
  （静か・落ち着く・心地よい・弾む・激しい は BPM と音量から自動で決まるもの）
- 二つは重ねて効く。選んだ範囲は端末に憶える（`pm.jbgenre` / `pm.jbmood`）
- 「おまかせ」も選んだ範囲の中から掛かる
- 当てはまる盤が無いときは、そう言って止まる（無言で空にしない）

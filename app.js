...
const VD = {
  ...
  art(x, w, h, al) {
    ...
    if (discKind === 'lp') {
      ...
      /* 曲の切れ目にある、少し広い輪 */
      for (let k = 1; k <= 4; k++) {
        const rr2 = r0 + (r1 - r0) * (k / 5);
        x.strokeStyle = 'rgba(0,0,0,.55)'; x.lineWidth = Math.max(1.5, dr * 0.008);
        x.beginPath(); x.arc(0, 0, rr2, 0, 7); x.stroke();
      }
      x.lineWidth = 1;
      ...
      x.fillStyle = vf; x.fillRect(-dr, -dr, dr * 2, dr * 2);
      x.restore();
      ...
      x.restore();
      /* 芯の穴 */
      x.beginPath(); x.arc(0, 0, dr * 0.032, 0, 7);
      x.fillStyle = 'rgba(6,6,8,.95)'; x.fill();
      /* 縁 */
  ...
  loadIndexFromCloud()
    .then(ok => {
      ...
      if (ok) { note('控えを取り込んだ'); renderRoute(); }
        ...
        /* 控えがまだ無い。**開いただけでは上がらない**ので、ここで作る。
           これが無いと、ジャケットや★を集めた端末で「何かを変える」まで
           pCloud に何も上がらず、別の端末では空のまま見えることになる。
           （実際、住所を変えたらジャケットが1527枚ぶん見えなくなった） */
        note('控えがまだ無い。いまの中身で作る');
        cloudSoon(1500);
      }
    })
